...
export function PaymentForm({ isLoading, isSubmitting, disabled, onPay, showVerification, paymentSuccess }: PaymentFormProps) {
  return (
    <div className={showVerification || paymentSuccess ? 'hidden' : ''}>
      ...
      <label className="flex items-start gap-3 p-4 border border-gray-300 rounded-md mb-8 cursor-pointer hover:bg-gray-50 transition-colors">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Pay faster on HalalPay merchants and everywhere Link is accepted.</p>
        </div>
      </label>
